...
# Primary model: sharper captions, but a tight free-tier daily cap.
# Fallback model: proven to work at 12-clip scale. Used automatically if the
# primary is rate-limited or refused, so a quota wall degrades to a real
# caption instead of generic filler.
...
N_FRAMES = 14           # frames sampled per clip (clips run 30s-2min)
FRAME_WIDTH = 768       # downscale: enough detail for the judge, still fast
...
STYLE_RULES = """\
formal            - Professional, objective, factual. Name what is concretely visible.
                    e.g. "A cyclist crosses a rain-slicked intersection as traffic waits at the light."
sarcastic         - Dry, ironic, deadpan understatement. Mock the situation, never a person.
                    e.g. "Ah yes, another thrilling episode of Man Waits For Light To Change."
humorous_tech     - Funny, with ONE programming or technology metaphor that genuinely maps to
                    what is on screen. The joke must only work for THIS clip.
                    e.g. "He's been stuck at this red light so long he's basically deadlocked."
humorous_non_tech - Funny, everyday observational humour. ZERO technical or computing words.
                    Not one. No code, servers, software, rendering, buffering, loading, .exe.
                    e.g. "The rain has clearly decided that today, everyone gets a free shower."

Be specific and vivid — name the actual subject, colours, weather, and action.
A caption that could describe any random video scores zero.
Commit to the joke: a hedged joke isn't funny and won't score.
"""
...
def call_gemini(frame_paths, model=None):
...
def parse_captions(text):
    ...
    try:
        obj, _ = json.JSONDecoder().raw_decode(text)   # ignores any trailing junk
...
def process(task):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as wd:
            ...
            caps = {}
            # Attempts 1-2 use the primary model. If it is refused (quota) or
            # keeps failing, attempts 3-4 switch to the fallback model, which
            # has proven headroom. A real caption beats a perfect one we never get.
            for attempt in (1, 2, 3, 4):
                model = MODEL if attempt <= 2 else FALLBACK_MODEL
                try:
                    caps = call_gemini(frames, model)
                    if all(s in caps for s in styles):
                        if attempt > 2:
                            log(f"{tid}: captioned via fallback model")
                        break
                except urllib.error.HTTPError as e:
                    if e.code == 429:
                        # Primary is capped -> go straight to the fallback model
                        # rather than burning the clock on a backoff that cannot help.
                        if attempt <= 2:
                            log(f"{tid}: {model} rate limited -> switching to {FALLBACK_MODEL}")
                            continue
                        wait = min(15 * (attempt - 2), remaining() - 40)
                        log(f"{tid}: rate limited, backing off {wait:.0f}s")
                        if wait <= 0:
                            break
                        time.sleep(wait)
                        continue
                    log(f"{tid}: attempt {attempt} ({model}) failed: {e}")
...
def main():
    ...
    log(f"primary={MODEL}  fallback={FALLBACK_MODEL}  frames={N_FRAMES}")
